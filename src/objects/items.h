/*
 * Items object script: apples (the dog grows) and pears (the dog
 * shrinks, never past the minimum three-piece shape).  Pure state +
 * placement; the dog script decides what eating means.
 */
 ...
 * (core/undo.c). */
typedef enum ItemKind { KIND_APPLE, KIND_PEAR, KIND_COUNT } ItemKind;
...
int pear_count(void);
bool pear_alive(int index);
uint16_t pear_cell(int index);
int pear_index_at(uint16_t cell); /* -1 when none */
void pear_consume(int index);
